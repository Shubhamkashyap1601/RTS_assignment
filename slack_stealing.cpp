...
ofstream outputFile("stealing.csv");
...
void ScheduleTasks(vector<Task>& order, vector<AP_Task>& aperiodic_tasks) {
  ...
  int n = order.size();
  int time = 0;
  int slack = MAX_TIME; // Initialize slack to maximum time
  ...
  int ai = 0;
  while(time <= MAX_TIME) {
    // Add tasks based on release time
    ...
    if (!pq.empty()) {
      Task temp = pq.top();
      pq.pop();
      // Update slack with next task's deadline (if it exists)
      if (!pq.empty()) {
        slack = pq.top().deadline - time;
      } else {
        slack = MAX_TIME - time; // If no next task, slack is remaining time
      }
      ...
      // Check for aperiodic tasks even if priority queue is not empty
      if (!apq.empty() && slack > apq.top().execution) {
        AP_Task temp = apq.top();
        apq.pop();
        outputFile << time << "\t\t\tAP" << temp.id << "\n";
        temp.execution--;
        slack -= temp.execution; // Decrement slack after scheduling aperiodic task
        if (temp.execution > 0) {
          apq.push(temp);
        }
      }
...
int main() {
    ...
    cout << "Scheduled saved in stealing.csv\n";
    return 0;
}
